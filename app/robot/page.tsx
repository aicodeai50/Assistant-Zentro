...
export default function RobotPage() {
  ...
  return (
    <>
      <RobotBootOverlay open={booting && enabled} onDone={() => setBooting(false)} />
      ...
      <main className="mx-auto max-w-5xl px-6 py-14 text-neutral-100">
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <button
            onClick={() => {
              setEnabled((v) => {
                const next = !v;
                if (next) setBooting(true); // boot again when re-enabled
                return next;
              });
            }}
          ...
          <button
            onClick={() => enabled && setBooting(true)}
            disabled={!enabled}
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1.3fr_.7fr]">
          {enabled ? (
            ...
            <div className="rounded-3xl border border-neutral-800 bg-black p-10 text-neutral-400">
              ...
              <br />
              Turn it on to experience the cinematic AI assistant.
            </div>
          )}

          {/* Optional side panel (if you already have it) */}
        ...
        <section className="mt-10 rounded-2xl border border-neutral-800 bg-neutral-950 p-6">
          ...
          <ul className="mt-3 list-disc space-y-2 pl-5 text-neutral-400">
            ...
            <li>Cinematic boot helps trust + delight without blocking the product.</li>
          </ul>
        </section>
      </main>
    </>
