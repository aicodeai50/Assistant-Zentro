...
export default async function SecurityDashboard() {
  ...
  return (
    <div className="p-8 space-y-8">
      ...
      <section className="space-y-2">
        <h2 className="text-lg font-semibold">Ì∑† LLM Anomalies</h2>
            ...
            {data.anomalies.map((a: any, i: number) => (
              <div key={i} className="border rounded-lg p-3 bg-gray-50">
                ...
                <div className="mt-1 text-sm">
                  <div><b>Severity:</b> {a.metadata?.severity ?? "unknown"}</div>
                  <div><b>Confidence:</b> {a.metadata?.confidence ?? "n/a"}</div>
                  <div><b>Action:</b> {a.metadata?.recommended_action ?? "n/a"}</div>
                  <div><b>Reason:</b> {a.metadata?.reason ?? "n/a"}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="space-y-2">
        <h2 className="text-lg font-semibold">Ì≥à Recent Usage</h2>
        <div className="max-h-64 overflow-auto border rounded-lg p-3 text-xs font-mono bg-white">
          {(data.usage || []).map((u: any, i: number) => (
            <div key={i}>
              [{new Date(u.timestamp * 1000).toISOString()}] {u.endpoint} ‚Äî {u.api_key} ‚Äî tokens:{u.tokens}
            </div>
          ))}
        </div>
      </section>
