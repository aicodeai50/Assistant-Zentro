...
export default function CareerLaunchpadPage() {
  ...
  const [goal, setGoal] = useState(init.q);
  ...
  async function run() {
    ...
    const prompt = [
      ...
      "Room: Career Launchpad",
      "",
      `User goal: ${g}`,
      "",
      "Deliver:",
      "- 7-day plan (daily actions)",
      "- 10 interview questions (role-aligned) + what a strong answer looks like",
      "- 1 mini project idea",
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <div className="mt-2 text-sm text-white/70">
              Turn learning into outcomes: interview practice, plans, and next steps.
            </div>
          </div>
        ...
        <div className="mt-6 rounded-3xl border border-white/15 bg-black/35 p-6 backdrop-blur-xl">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            <div className="rounded-2xl border border-white/15 bg-white/5 p-5">
              <div className="text-xs tracking-widest text-white/60">YOUR GOAL</div>
              <div className="mt-2 text-sm text-white/70">
                Type your target role or outcome and get a plan + interview practice.
              </div>

              <input
                value={goal}
                onChange={(e) => setGoal(e.target.value)}
                placeholder="e.g., internship in cybersecurity / med placement / consulting"
            ...
            <div className="rounded-2xl border border-white/15 bg-white/5 p-5">
              ...
              {!out ? (
                <div className="mt-3 text-sm text-white/70">Your plan appears here.</div>
              ) : (
          ...
          <div className="mt-5 text-xs text-white/50">
            Purpose: translate study into real outcomes (interviews, readiness, next steps).
          </div>
        </div>
      </div>
    </div>
