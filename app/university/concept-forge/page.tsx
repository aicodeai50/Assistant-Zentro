...
export default function ConceptForgePage() {
  ...
  async function run() {
    ...
    const prompt = [
      ...
      "",
      "Deliver:",
      "- A concept map as bullets: Core → Subconcepts → Dependencies",
      "- 5 'If this changes, what breaks?' questions",
      "- 5 analogies to build intuition",
      "- A short 'teach-back' prompt (what the learner should explain)",
    ].join("\n");
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <div className="mt-2 text-sm text-white/70">
              Turn complex topics into clear systems: dependencies, structure, and intuition.
            </div>
          </div>
        ...
        <div className="mt-6 rounded-3xl border border-white/15 bg-black/35 p-6 backdrop-blur-xl">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            <div className="rounded-2xl border border-white/15 bg-white/5 p-5">
              <div className="text-xs tracking-widest text-white/60">MAP A TOPIC</div>
              <div className="mt-2 text-sm text-white/70">
                Enter a topic and get a structured concept map + intuition builders.
              </div>

              <input
                value={topic}
                onChange={(e) => setTopic(e.target.value)}
                placeholder="e.g., React state / supply chain / immune response"
            ...
            <div className="rounded-2xl border border-white/15 bg-white/5 p-5">
              ...
              {!out ? (
                <div className="mt-3 text-sm text-white/70">Your concept map appears here.</div>
              ) : (
          ...
          <div className="mt-5 text-xs text-white/50">
            Purpose: make hard topics “click” by revealing structure and dependencies.
          </div>
        </div>
      </div>
    </div>
