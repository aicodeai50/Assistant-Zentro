...
def lock_key(api_key: str, reason: str):
    ...
    audit_log(
        ...
        metadata={
            "reason": reason,
            "lock_minutes": LOCK_DURATION_SECONDS // 60,
        },
    )

    send_alert(
        title="íº¨ API Key Locked",
        message=f"Key {api_key[:6]}*** locked\nReason: {reason}",
    )
