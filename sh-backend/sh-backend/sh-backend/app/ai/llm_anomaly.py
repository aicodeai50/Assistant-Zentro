...
ALERT_ON_SEVERITY = set(
    s.strip().lower()
    for s in os.getenv(
        "LLM_ANOMALY_ALERT_SEVERITY", "high,critical"
    ).split(",")
    if s.strip()
)
...
def _send_slack(finding):
    send_alert(
        "í·  LLM Anomaly Detected",
        f"Actor: {finding.get('actor')}\n"
...
def run_llm_anomaly_detection():
    ...
    user_prompt = (
        "Analyze the following per-actor usage summary.\n\n"
    ...
    for f in findings:
        ...
        if f.get("severity", "").lower() in ALERT_ON_SEVERITY:
            _send_slack(f)

        if (
            ENABLE_LLM_AUTOLOCK
            and f.get("recommended_action") == "lock_key"
            ...
            lock_key(f["actor"], f"LLM autolock: {f.get('reason')}")
